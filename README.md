...
- **Supabase Storage** — Photos and documents uploaded to Supabase Storage via API routes
- **Supabase database** — PostgreSQL for employees, drafts, submissions, and HR data
...
| `SUPABASE_STORAGE_BUCKET` | Storage bucket name (default: `onboarding-documents`) |
...
### 3. Create Supabase tables & storage bucket

Run the SQL in `supabase/schema.sql` in your Supabase project **SQL Editor**. This creates all tables and the private `onboarding-documents` storage bucket.
...
| `/api/onboard/[token]/upload` | POST | Upload file to Supabase Storage |
...
| `/api/files/[...fileKey]` | GET | Redirect to signed Supabase Storage URL |
